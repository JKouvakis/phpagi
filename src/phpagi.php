...
class AGI {
     ...
     */
    function __construct($config = null, $optconfig = [], $socket = null) {
        // load config
    ...
    function channel_status($channel = "") {
        ...
        switch ($ret["result"]):
            case -1:
                $ret["data"] = trim(
                    "There is no channel that matches $channel"
                );
                break;
            case AST_STATE_DOWN:
    ...
    function get_data($filename, $timeout = null, $max_digits = null) {
        return $this->evaluate(
            rtrim("GET DATA $filename $timeout $max_digits")
        );
    }
    ...
    function record_file(
        ...
        $cmd = trim(
            "RECORD FILE $file $format \"$escape_digits\" $timeout $offset"
        );
        if ($beep) {
            $cmd .= " BEEP";
        }
    ...
    {
        return $this->evaluate(
            "STREAM FILE $filename \"$escape_digits\" $offset"
        );
    }
    ...
    function exec_dial(
        ...
        return $this->exec(
            "Dial",
            trim("$type/$identifier,$timeout,$options,$url", ",")
        );
    }
     ...
     * @return array, see evaluate for return information.
     */
    function fastpass_say_punctuation(
        &$buffer,
        $text,
        $escape_digits = "",
        $frequency = 8000
    ...
    {
        $this->asmanager = new AGI_AsteriskManager(null, $this->config);
        $this->asmanager->pagi = &$this;
        $this->config = &$this->asmanager->config;
    ...
    function evaluate($command)
    {
        ...
        $count = 0;
        do {
            $str = is_null($this->socket)
                ? @fgets($this->in, 4096)
                : @socket_read($this->socket, 4096, PHP_NORMAL_READ);
        } while ($str == "" && $count++ < 5);
    ...
    function conlog($str, $vbl = 1)
    {
        ...
        if (
            isset($this->config["phpagi"], $this->config["phpagi"]["debug"]) &&
            $this->config["phpagi"]["debug"] != false
        ) {
            if (!$busy) {
    ...
    function which($cmd, $checkpath = null)
    {
        ...
        foreach (explode(PATH_SEPARATOR, $chpath) as $path) {
            if (
                !function_exists("is_executable") ||
                is_executable($path . DIRECTORY_SEPARATOR . $cmd)
            ) {
